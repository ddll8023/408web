...
后端 `exam.py` 是路由入口，`ExamService` 直接使用异步 SQLModel Session，没有 Repository 层；`ExamResponse` 是 API DTO，Service 中 `_to_response()` 负责科目、作者查询和分类 JSON 解析。前端页面通过 `frontend/src/api/exam.ts` 调用统一 `request.ts`，题目卡片使用 `MarkdownViewer`，编辑器由 `ExamEditDialog` 提供。
...
- **写入并发**：创建/更新先由 Service 做友好查重，数据库模型和迁移脚本已提供 `question_number IS NOT NULL` 条件下的 `(year, question_number)` 唯一索引；唯一冲突仍需按 409 处理，避免把并发冲突误报为成功。
...
- **Schema 初始化**：应用启动时执行 `SQLModel.metadata.create_all`；现有数据曾由 `scripts/migrate_high_priority.py` 和 `schema_migrations` 记录完成高优先级迁移。后续模型结构变化仍需按项目授权规则处理，删除/重建已有数据库必须先获用户明确授权。
...
| `difficulty` | `TEXT` | 可空 | `EASY`/`MEDIUM`/`HARD`，模型和迁移包含检查约束。 |
| `author_id` | `INTEGER` | 非空 | 外键 `user.id`；记录创建者。 |

### 3.3 关系、索引和唯一性

- `Subject 1:N ExamQuestion`，`User 1:N ExamQuestion`；当前题目不直接外键引用 `ExamCategory`。
- 已确认模型声明 `(year, question_number)` 非空部分唯一索引；迁移脚本另建立 `subject_id`、`author_id` 普通索引。列表仍常按年份、科目过滤并按更新时间/题号排序。
...
- 当前只有高优先级数据迁移脚本和 `schema_migrations` 记录，没有完整通用迁移框架；结构变化不能在未授权情况下删除或重建当前 `web408.db`。
...
上述 `ExamYearQueryRequest` 等名称是目标契约示例；当前实际路由和模型名称以 `backend-fastapi/app/api/v1/exam.py`、`app/schemas/exam.py` 及 `frontend/src/api/exam.ts` 为准，不得用无约束 `body: dict` 代替 Request Schema。

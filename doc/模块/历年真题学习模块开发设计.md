...
4. Markdown、数学公式、代码、SVG 和图片在题干、选项和答案中均可安全展示；PNG、SVG 等不同格式的插图按内容角色和自然比例收缩，纵向/稀疏图不能仅因达到保护上限就压过正文。
5. 复制内容包含题号/题型/分类及对应题干、选项或答案；支持 Markdown、纯文本和 PNG 图片，图片范围为题目+选项、答案或全部，页面与复制的插图相对正文比例一致，复制标题和标签不被拆行；空内容、图片生成失败和剪贴板不支持均有明确反馈。
...
5. 复制由 `QuestionCopyMenu.vue` 处理命令，图片复制按“题目+选项、答案、全部”三种范围通过 `QuestionImageRenderer.vue` 复用 `MarkdownViewer` 生成 PNG 并写入图片剪贴板；截图节点使用与阅读端相同的正文/选项媒体规则，不能因隐藏渲染而取消图片约束；剪贴板能力不可用时下载 PNG。Markdown/Word 导出由浏览器生成，按科目导出当前直接访问后端文件响应。
...
- 编辑预览、阅读和 PNG 复制共用 `contentRole`；当前 384×288px/140×112px 是保护性上限，正文 18em×12em、选项 8em×6em 是待浏览器校准的候选值。2009 年真题第 3 题的 890×1024 PNG 作为纵向稀疏图回归样例，不能把它误归为 SVG 问题。
- 单图可使用 `width`、`media-wide` 或 `media-inline` 覆盖默认策略，具体写法见题库内容维护模块设计。图片与可规范化 SVG 的原尺寸预览只属于阅读交互，不参与 PNG 复制，也不触发选项作答；复制版式的标题和标签应保持单行。

...
| 后端入口 | `backend-fastapi/app/api/exam.py` | 暴露真题查询、统计、导出和管理员 CRUD。 |
| 业务/数据 | `backend-fastapi/app/services/exam_service.py`、`app/services/exam_query_service.py`、`app/services/exam_command_service.py`、`app/repositories/exam_repository.py`、`app/schemas/exam.py`、`app/models/entities.py` | 查询、分页、分类 JSON 转换、统计、导出和 `exam_question` 持久化。 |
| 上游/下游 | 学科目录、账户权限、题库维护、统计导出、收藏和图片资源 | 提供筛选上下文、管理员身份、题目事实、派生统计、收藏跳转和图片访问。 |

### 2.2 当前代码边界

后端 `exam.py` 是路由入口，`ExamService` 保留兼容门面，查询/写入/导出 Service 通过 `ExamRepository` 使用异步 SQLModel Session；`ExamResponse` 是 API DTO，查询 Service 中 `to_response()` 负责科目、作者查询和分类 JSON 解析。前端页面通过 `frontend/src/api/exam.ts` 调用统一 `request.ts`，题目卡片使用 `MarkdownViewer`，编辑器由 `ExamEditDialog` 提供。
...
3. 当前 `exam.py` 查询、导航、统计、详情、导出及维护路由均使用 POST + Request Schema；统一 JSON 响应为 `ApiResponse`，分页数据为 `lists`/`pagination`。第 4 节仍保留未来公开契约的命名建议，不能将目标路径当作现有路由。
...
- **访问方式**：`ExamService` 使用异步 SQLModel `AsyncSession`；Session 由 `SessionDep` 注入，每请求创建并关闭，写用例显式提交，异常时回滚。
...
- `backend-fastapi/app/api/exam.py` 当前提供：分页查询、按年份查询、按科目查询分类、年份统计、完整/轻量导航索引、分类统计、按科目分类查询、按科目导出、详情、查重、创建、更新和删除。
...
上述 `ExamYearQueryRequest` 等名称是目标契约示例；当前实际路由和模型名称以 `backend-fastapi/app/api/exam.py`、`app/schemas/exam.py` 及 `frontend/src/api/exam.ts` 为准，不得用无约束 `body: dict` 代替 Request Schema。
...
- 当前按分类统计页面的“统计导出”按钮处于禁用状态，原因是后端尚未提供对应的 `category-stats/export` 路由；页面不再直接调用该地址。本模块只设计真题导出，统计导出见“统计与导出模块”文档。

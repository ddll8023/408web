...
6. 用户端改编题页面按科目与分类可浏览，卡片展示来源年份与题号，空结果与加载失败有明确状态。
7. 分类改名后改编题分类标签同步为新名称；删除仍被改编题引用的分类被拒绝。
8. 改编题正文引用的图片在图片资源管理中显示为「已引用」，未引用清理不会删除该文件。
...
- 用户端只调用 `/query`、`/{id}/detail`、`/subject-stats`、`/categories/{subject_id}` 与目录模块的分类树接口，不调用来源占用检查与来源维护接口。

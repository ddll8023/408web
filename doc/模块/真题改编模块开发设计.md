...
本模块的目标是：为改编题提供独立的题目库和来源引用关系，并让真题侧的改编覆盖情况可查询，使用户端可以浏览改编题及其来源标注。
...
| F3 | 记录小问 | 来源行可填写小问/备注（如 `41(2)`），整题粒度留空 |
| F4 | 来源即时解析 | 录入表单填写完年份与题号后即可看到该真题是否存在及题目摘要；未命中时给出显式提示 |
| F5 | 查重 | 同一「标题 + 题号」的改编题被拒绝并提示已存在；同一真题已被其他改编题引用时给出提示但不阻断 |
| F6 | 来源有标注可筛选 | 管理端列表可区分「已标注来源」与「未标注来源」的改编题，未标注不会被静默忽略 |
| F7 | 真题侧反查 | 输入某年某题，可查询引用它的全部改编题（标题、题号、题型、小问） |
| F8 | 改编覆盖统计 | 按年份（可限定科目）给出真题总题数、已改编题数、未改编题号，并单独报告悬空来源数量 |
| F9 | 用户端浏览 | 用户可按科目与分类浏览改编题，卡片展示「改编自 XXXX 年第 N 题」 |
| F10 | 分类与图片联动 | 分类改名后改编题分类标签同步；分类删除校验计入改编题引用；改编题正文引用的图片被识别为已引用，不被未引用清理删除 |

### 1.3 非功能需求

- **权限**：查询类接口对匿名用户开放，与现有真题、模拟题查询一致；创建、更新、删除、查重接口必须经 `get_current_admin` 认证依赖，服务端权限为唯一安全边界，前端界面隐藏不作为授权手段。
...
2. 管理端改编题录入、编辑、删除、查重与来源维护。
3. 真题侧反查接口与改编覆盖统计接口，以及管理端覆盖统计页面。
4. 用户端改编题浏览页面（按科目与分类）。
5. 分类联动（`question_type` 取值、改名同步、删除保护、分类统计计数）与媒体图片引用扫描接入。
...
| 前端管理端/用户端 | 复用现有表格、弹窗、表单、Markdown 渲染与请求封装 | 不新建请求实例、不绕过统一响应封装 |

### 1.6 验收标准

1. 录入来源「2021 年 + 15 题」时，若真题库存在该题则显示原题摘要；填写「2021 年 + 99 题」时提示该题号不存在，仍可保存，并在详情中标记为未解析来源。
2. 同一改编题重复添加同一来源行被唯一约束拦截；不同改编题引用同一真题允许保存，并在查重响应中提示已被引用。
3. 同一「标题 + 题号」的改编题再次创建返回冲突错误；管理端提示已存在的改编题。
4. 反查「2021 年 15 题」返回全部引用它的改编题，包含标题、题号、题型与小问备注。
5. 覆盖统计中 2021 年显示真题总数 47、已改编题数与未改编题号列表；来源中无法对应真题库的引用数单独报告。
6. 删除某改编题后，其来源行随之删除，覆盖统计与反查结果同步减少。
7. 用户端改编题页面按科目与分类可浏览，卡片展示来源年份与题号，空结果与加载失败有明确状态。
8. 分类改名后改编题分类标签同步为新名称；删除仍被改编题引用的分类被拒绝。
9. 改编题正文引用的图片在图片资源管理中显示为「已引用」，未引用清理不会删除该文件。

### 1.7 待确认事项

| 事项 | 当前处理 | 影响 |
|---|---|---|
| 用户端是否允许从「改编自 XXXX 年第 N 题」跳转到真题原文 | 设计为展示来源文案，暂不提供跳转链接 | 若需跳转，需复用真题详情接口并按公开范围确认版权口径 |
...
flowchart LR
    subgraph FE[前端]
        ...
        FE3[用户端 改编题浏览 AdaptationClassify.vue]
        FE4[管理端 真题管理 ExamManage.vue]
    end
...
| `schemas.py` | 查询参数、来源输入/输出、创建与更新请求、响应、查重、反查、覆盖统计的 Pydantic 模型 |
| `repository.py` | 改编题分页、查重、按来源反查、来源行查询与替换、批量计数等持久化访问，只访问本模块两张表 |
...
| `read_service.py` | 对 `catalog` 公开分类引用、题目计数与科目计数的不可变 DTO 边界 |
...
| `frontend/src/components/business/AdaptationSourceEditor.vue` | 受控来源编辑区：多行「年份 + 题号 + 小问备注」，增删行，调用解析接口回填原题摘要与命中状态 |
| `frontend/src/views/admin/AdaptationManage.vue` | 管理端列表、筛选、分页、排序、编辑弹窗（含来源编辑区）、删除、查重提示；复用 `useAdminTable`、`Table`、`Dialog`、`Pagination`、`InputSelect`、`MultiSelectCascader`、`useQuestionForm`、`useJsonImport` |
| `frontend/src/views/admin/AdaptationCoverage.vue` | 覆盖统计页：年份 × 科目维度的总数、已改编数、未改编题号，点击查看引用该题的改编题 |
| `frontend/src/views/user/AdaptationClassify.vue` | 用户端浏览：科目入口、分类大纲、题目列表，卡片展示来源标注；复用 `CategoryOutline`、`ExamQuestionCard`、`MarkdownViewer` |
| `frontend/src/router/index.ts`、`components/business/Navigation.vue` | 新增 `/manage/adaptation`、`/manage/adaptation-coverage`、`/adaptation` 路由与管理端/用户端菜单入口 |
...
flowchart TD
    ...
    C --> D[查重: 标题 + 题号]
    D -->|已存在| E[返回 409 冲突]
    D -->|通过| F[批量解析来源, 写 exam_question_id 或留空]
    F --> G[写入改编题与来源行]
    G -->|唯一约束冲突| H[集中映射为冲突错误]
    G -->|成功| I[提交事务并返回详情]
```
...
- 创建：请求 → 结构校验（Pydantic）→ 业务校验（科目分类、题型结构、查重）→ 来源解析 → 两张表写入 → 提交 → 响应。
...
- 读取：用户端与管理端列表都走同一查询用例，区别仅在筛选条件与响应字段裁剪，避免两套查询口径。
...
| 事实键用「年份 + 题号」，`exam_question_id` 仅作解析结果 | 真题库可能缺题或被删，事实键必须独立成立；同时保留 `exam_question_id` 才能一跳直达原题 | 只用外键则真题缺题时无法录入；只用事实键则每次跳转都要再解析，且真题改年份/题号后无法感知 |
...
| `adaptation_source` | 一条改编题到一道真题的来源引用（年份、题号、小问、解析结果） | 不保存真题正文，不保存改编关系说明文本 |
...
| `title` | TEXT | 可空 | 题集/标题分组，沿用模拟题 `title` 语义 |
| `question_number` | INTEGER | 可空 | 题集内题号，用于排序与查重 |
...
| `source_part` | TEXT | 非空，默认 `''` | 小问或备注，空字符串表示整题 |
| `exam_question_id` | INTEGER | 可空 | 解析命中的真题 ID，外键 `exam_question.id` |

`source_part` 采用「非空 + 默认空串」而不是可空列，原因是 SQLite 唯一约束中 `NULL` 之间永不相等：可空列会让同一改编题重复引用同一整题的两行同时存在，重复无法被数据库拦截。
...
CREATE UNIQUE INDEX uq_adaptation_question_title_number
    ON adaptation_question (title, question_number)
...
CREATE TABLE adaptation_source (
    ...
    CONSTRAINT uq_adaptation_source_ref UNIQUE
        (adaptation_id, source_year, source_question_number, source_part)
);
...
- 来源事实键（年份、题号、小问）一旦写入不随真题变化自动改写；真题被删只清空 `exam_question_id`。
- 不保留操作日志与审计表；作者与时间字段足以定位最近维护人。
- 该表数据由管理员在管理端维护，用户端只读；导出能力本次不涉及，后续如需导出应复用 `reporting` 模块的导出编排，而不是在本模块内直接生成文件。

### 3.8 事务边界与一致性

- 写用例逐次提交，来源替换与主体写在同一事务内；来源的「全量替换」在事务内先删后插。
- 一致性要求：来源行的 `source_question_number` 必须落在 `1—1000` 区间（与真题表题号约束一致），`source_year` 落在 `1990—2100`；这两条在 Schema 与数据库 CHECK 之外的应用校验中确认，避免非法年份进入覆盖统计。
...
- 鉴权：查询类接口不挂鉴权依赖（与真题、模拟题查询一致）；写入、删除、查重接口使用 `AuthUser = Depends(get_current_admin)`，作者取 `admin.user_id`。
...
| 科目题目统计 | `/api/adaptation/subject-stats` | 匿名 | 无 | `ApiResponse[list[AdaptationSubjectStat]]` |
...
| 查重 | `/api/adaptation/check-duplicate` | 管理员 | `AdaptationDuplicateRequest` | `ApiResponse[AdaptationDuplicateCheckResponse]` |
...
| `source_question_number` | int \| None | 1—1000 | 按来源题号筛选 |
| `source_state` | `all` \| `with_source` \| `without_source` | 默认 `all` | 是否已标注来源 |
| `sort_field` | Literal | `id` / `update_time` / `question_number` / `title` | 排序字段白名单 |
...
| `source_question_number` | int | 1—1000，必填 | 来源真题题号 |
| `source_part` | str \| None | ≤50 | 小问/备注，服务端归一化为空串存库 |

**创建请求 `AdaptationCreateRequest`（继承 `QuestionCreateFields`）**

在共用字段之外新增：`title`（≤200）、`question_number`（1—1000）、`sources: list[AdaptationSourceRefInput]`（≤20 条，允许为空数组表示暂未标注来源）。
...
```json
{
  ...
  "sources": [
    ...
    { "source_year": 2022, "source_question_number": 3, "source_part": "3(2)" }
  ]
}
```

**响应 `AdaptationResponse`**：主体字段与 `ExamResponse` 对齐（`id`、`title`、`question_number`、`question_type`、`content`、`options`、`answer`、`category`、`subject_id`、`subject_name`、`difficulty`、`author_id`、`author_name`、`create_time`、`update_time`），另加：
...
|---|---|---|
| `id` | int | 来源行 ID |
| `source_year` / `source_question_number` / `source_part` | int / int / str | 事实键 |
| `exam_question_id` | int \| None | 解析命中的真题 ID |
| `source_exists` | bool | 真题库中是否命中（由解析结果决定，不额外查询） |
| `exam_title` | str \| None | 命中的真题标题，用于展示与跳转 |

**来源解析 `AdaptationSourceLookupItem`**：`source_year`、`source_question_number`、`source_part`、`exists`、`exam_question_id`、`exam_title`、`exam_question_type`。请求体为 `{ "sources": [...] }`，一次最多 20 条，服务端一次查询取回全部命中项。

**反查 `AdaptationBySourceRequest` / `AdaptationBySourceItem`**

- 请求：`source_year`、`source_question_number` 必填，`subject_id` 可选（仅用于统计视角过滤）。
- 响应项：`id`、`title`、`question_number`、`question_type`、`subject_id`、`subject_name`、`source_part`、`update_time`。
...
| `source_question_number` 不在 1—1000、年份越界 | 请求校验 | 422 | Pydantic 字段约束 |
| 同一标题 + 题号已存在 | `ConflictException` | 409 | 「该标题下题号 N 已存在」 |
| 重复引用同一来源（含小问） | `IntegrityError` 映射 | 409 | 「来源重复」 |
...
- 创建不是幂等操作，重复提交由唯一 `(title, question_number)` 兜底为 409。
...
- 用户端只调用 `/query`、`/{id}/detail`、`/subject-stats`、`/categories/{subject_id}` 与目录模块的分类树接口，不调用查重与来源维护接口。

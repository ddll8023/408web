...
内容维护者正在把真题改编为新的练习题目，录入时必须记录这批改编题来自「哪一年、哪一道或哪几道真题」。现有两个题库都无法承载这一信息：真题表自身的 `year` / `question_number` 表示真题本体，模拟题表的 `source` 字段语义是来源机构，把改编题塞进模拟题库会混用统计口径，也无法完整维护多来源关系。

本模块的目标是：为改编题提供独立的题目库和来源引用关系，支持管理端维护来源、解析状态与来源占用提示。
...
| F7 | 分类与图片联动 | 分类改名后改编题分类标签同步；分类删除校验计入改编题引用；改编题正文引用的图片被识别为已引用，不被未引用清理删除 |
| F8 | 改编题公开阅读 | 学习者可从 `/adaptation` 按科目/分类浏览改编题；从真题关联弹窗新标签页打开后，页面滚动并高亮对应题目 |
...
- **性能**：改编题列表必须分页；来源查询按索引执行，来源解析批量进行，避免逐行查询造成的 N+1。
- **可靠性**：来源未命中真题库时不允许阻断录入，也不允许静默丢弃事实键；详情中必须保留未解析来源状态。
...
3. 分类联动（`question_type` 取值、改名同步、删除保护、分类统计计数）与媒体图片引用扫描接入。
4. 学习端改编题阅读页、科目/分类导航、题目锚点定位、来源查看和真题关联跳转。
...
| `exam` | 只读解析来源「年份 + 题号」，用于录入提示与来源状态展示 | 不写入真题表，不修改真题表结构与真题接口行为 |
...
3. 删除某改编题后，其来源行随之删除，详情与来源占用提示同步反映当前数据。
4. 分类改名后改编题分类标签同步为新名称；删除仍被改编题引用的分类被拒绝。
5. 改编题正文引用的图片在图片资源管理中显示为「已引用」，未引用清理不会删除该文件。
...
flowchart LR
    subgraph FE[前端]
        ...
        FE2[学习端 改编题阅读 AdaptationClassify.vue]
    end
...
| `schemas.py` | 查询参数、来源输入/输出、创建与更新请求、响应、来源占用检查的 Pydantic 模型 |
| `repository.py` | 改编题分页、来源行查询与替换、来源占用提示等持久化访问，只访问本模块两张表 |
| `mapper.py` | 实体到响应的纯字段转换，不含查询与事务 |
| `query_service.py` | 列表、详情、来源解析结果拼装、来源占用提示、图片引用文本等读取用例，不提交事务 |
...
| 文件 | 职责 |
|---|---|
| `frontend/src/types/adaptation.ts` | 后端 DTO 到前端领域模型（camelCase）的类型：改编题、来源引用、查询参数、创建/更新请求、解析结果 |
...
| `frontend/src/views/admin/AdaptationManage.vue` | 管理端列表、筛选、分页、排序、编辑弹窗（含来源编辑区）、删除、来源占用提示；复用 `useAdminTable`、`Table`、`Dialog`、`Pagination`、`InputSelect`、`MultiSelectCascader`、`useQuestionForm` |
| `frontend/src/views/user/AdaptationClassify.vue` | 学习端改编题阅读页：按科目/分类导航、题型筛选、分页加载、答案显隐、题目锚点和 hash 定位 |
| `frontend/src/components/business/AdaptationEntryCard.vue`、`AdaptationItemHeader.vue` | 组合改编题元信息、来源、复制、管理操作和通用题目卡片 |
| `frontend/src/components/business/AdaptationRelatedDialog.vue`、`AdaptationSourceDialog.vue` | 分别展示真题关联改编题和改编题关联真题，支持新标签页定位 |
| `frontend/src/components/business/QuestionJsonImportPanel.vue` | 真题、模拟题、改编题编辑弹窗共用的 JSON 导入交互；业务弹窗负责解析结果填充 |
| `frontend/src/router/index.ts`、`components/business/Navigation.vue` | 提供 `/adaptation`、`/manage/adaptation` 路由与用户/管理入口 |
...
- 读取：管理端列表、来源解析与来源占用提示都走本模块查询用例，避免多套查询口径。
...
| 选择 | 依据 | 替代方案与不采用原因 |
|---|---|---|
| 来源引用用独立子表 `adaptation_source` | 需求本质是「多行引用 + 来源占用检查」，子表可直接建索引、可做唯一约束与级联；项目已有 `mock_question_exam_mark` 这类独立表先例 | 主表两个字段无法表达多来源；`category` 那样的 JSON 数组列写法最省代码，但来源查询需要全表扫描并逐行解析 JSON，且无法用数据库约束拦截重复 |
...
| 依赖方向单向：`adaptation` → `exam.read_service` | 改编题来源解析由本模块通过真题只读边界完成，`exam` 只提供解析方法，避免 `exam` 反向依赖新模块 | 在真题查询响应里内嵌改编来源信息会让 `exam` 依赖 `adaptation`，并改变真题接口公开响应 |
...
当前结构 DDL 摘要（已按本项目开发期策略落地；时间列沿用 ORM 映射）：
...
| 索引 `(source_year, source_question_number)` | 来源占用检查与来源筛选（按来源维度） |
| 索引 `exam_question_id` | 按已解析真题 ID 核对来源 |
| 索引 `subject_id` | 管理端按科目筛选与分类统计 |

不额外建立 `source_question_number` 单列索引：该列的选择性低，来源查询始终与年份同时使用。
...
- 一致性要求：来源行的 `source_question_number` 必须落在 `1—47` 区间（与真题表题号约束一致），`source_year` 落在 `1990—2100`；这两条由 Schema 和服务端应用校验共同确认，避免非法来源进入题库。
...
- 本次已按授权重建现有 SQLite 数据库，物理移除 `adaptation_question.title`；改编题记录与来源记录保留，外键检查无错误。
- 后续修改字段时，仍需先提供数据库绝对路径、重建原因、数据丢失范围、新结构来源与检查方式，获得明确授权后才能执行。

### 3.10 规范落地

- `sqlite-ops`：连接级 `foreign_keys=ON`、WAL、`busy_timeout` 依赖现有实现；唯一性由数据库约束保证，应用校验只改善提示；索引来自真实查询模式（来源占用、按题取来源、按科目筛选）；开发期直接以当前定义建表，不建测试库与迁移链。
...
| `sort_field` | Literal | `id` / `update_time` / `create_time` | 排序字段白名单 |
...
**创建请求 `AdaptationCreateRequest`（继承不含标题的公共题目字段）**
...
**响应 `AdaptationResponse`**：主体字段与题目共用字段对齐（`id`、`question_type`、`content`、`options`、`answer`、`category`、`subject_id`、`subject_name`、`difficulty`、`author_id`、`author_name`、`create_time`、`update_time`），另加：
...
| `media.reference_reader` | `AdaptationQueryService.list_image_reference_texts()` | 只返回文本，不做文件操作；图片引用展示名使用 `[改编题] ID` |
| `exam` | 新增 `resolve_sources` 只读方法 | 不改变真题现有响应与接口 |

...
**已确认事实**：真题和模拟题分别保存于 `exam_question`、`mock_question`；前端使用 `ExamEditDialog.vue`、`MockEditDialog.vue`和 `useQuestionForm.ts`；后端分别由 `modules/exam/` 和 `modules/mock/` 的 Router、Query/Command Service 处理；新增/更新/删除路由使用管理员依赖。
...
| 方向 | 当前项目位置 | 协作内容 |
|---|---|---|
| 管理列表 | `frontend/src/views/admin/ExamManage.vue`、`MockManage.vue` | 分页、筛选、排序、查看、编辑和删除入口；分类筛选按当前科目加载已有分类，使用可搜索单选下拉，输入内容仅用于过滤候选项。 |
...
| 后端入口 | `backend-fastapi/app/modules/exam/router.py`、`modules/mock/router.py` | 真题/模拟题详情、查重和管理员 CRUD。 |
| 业务处理 | `backend-fastapi/app/modules/exam/command_service.py`、`modules/mock/command_service.py`、对应 Query Service | 查重、实体创建/更新/删除、目录校验、响应编排和事务提交。 |
| 数据契约 | `backend-fastapi/app/modules/exam/schemas.py`、`modules/mock/schemas.py`、`modules/question_content/schemas.py`、`modules/exam/models.py`、`modules/mock/models.py` | 请求/响应模型、共用题目内容规则以及两张题目表。 |
| 上游依赖 | 账户与权限、学科目录、图片资源 | 提供 ADMIN、科目/分类选择和图片 URL。 |
| 下游 | 历年真题学习、模拟题学习、统计与导出、图片引用治理 | 消费保存后的题目、分类 JSON 和 Markdown 内容。 |

### 2.2 当前代码边界与事实冲突

后端沿用 Router → Service → Repository → Model 的分层。`modules/exam/` 和 `modules/mock/` 已删除 `ExamService`/`MockService` 兼容门面，由 Query/Command Service 编排查询、写入和响应转换，数据库查询集中在对应 Repository；请求级 Session 负责生命周期，写用例由 Service 显式提交并在异常时回滚。
...
编辑器右侧预览、学习页和 PNG 复制统一由 `MarkdownViewer` 处理。真题、模拟题的选项编辑器传入 `content-role="option"`，其余内容使用正文角色。PNG、JPEG、GIF、WebP 和 SVG 不按文件格式分别指定视觉大小；格式只影响加载或 SVG 坐标处理。

当前实现的媒体尺寸是防溢出的保护性上限，不是所有图片的目标显示尺寸：

| 场景 | 字号/行高 | 当前保护上限 | 后续校准候选 |
|---|---|---:|---:|
| 题干、答案 | 16px / 1.6 | 384×288px | 18em×12em |
| 选项 | 14px / 1.5 | 140×112px | 8em×6em |
| 明确标记的行内符号 | 跟随内容角色 | 高约 1.4 倍字高 | 保持 |

候选值用于解决纵向、稀疏位图在达到高度上限后仍压过正文的问题；例如 2009 年真题第 3 题引用的是 890×1024 的 PNG，而不是 SVG。候选值必须通过页面、编辑预览和 PNG 复制的浏览器验收后才能替换当前保护上限。实际大小还受原图宽度、宽高比和容器约束；不主动放大小图，不根据像素尺寸猜测图像语义，也不自动裁剪源图白边。独立插图居中，选项插图左对齐，混排内容不强制换行。

默认 Markdown 图片无需修改。需要逐图调整时可在现有 Markdown 编辑区使用 HTML：

```html
<!-- 在默认上限内进一步缩小；width 支持正数、px、em、rem、%。 -->
<img src="/uploads/images/example.png" alt="矩阵" width="100">

<!-- 密集图允许更宽、更高；仍不超过原图和容器，不强制撑满。 -->
<img src="/uploads/images/example.png" alt="网络拓扑" class="media-wide" width="600">

<!-- 只有明确需要行内展示的符号才添加此类。 -->
状态 <img src="/uploads/images/example.png" alt="状态符号" class="media-inline"> 已更新。

<!-- SVG 保留原始 viewBox；单图宽度优先读取允许的内联 width，再读取 width 属性。 -->
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 620 180" class="media-wide" style="width: 500px;">
  <!-- 原有 SVG 内容 -->
</svg>
```

- SVG 仅调整已清洗的渲染 DOM：有 viewBox 时保留原点和坐标；只有明确固定宽高时才补缺失的 viewBox；信息不足或非法时不猜测。跳过 KaTeX 内部和嵌套 SVG。
- 图片和可规范化 SVG 支持鼠标/键盘打开可滚动的原尺寸弹窗；点击插图不同时提交选项作答。PNG 复制禁用交互，但保持同样的字号和媒体约束。
- 本次不新增数据库字段、图片裁剪或自动识图，不改原始图片文件；单图覆盖仅在管理员明确编辑对应 Markdown 时保存。

**浏览器验收清单（未执行）**：至少对照 2009 年真题第 3 题的纵向 PNG、“竟成 - 26 - 卷3”第 7 题矩阵选项、只有 viewBox 的 SVG、缺少 viewBox 的固定画布 SVG、长截图及 KaTeX 根号/括号；检查候选尺寸下图文比例、窄屏不溢出、无拉伸裁切、普通/宽图/行内规则、编辑与阅读比例、PNG 内容比例，以及复制标题和标签不被拆行。密集图缩小后仍需人工确认可读性，必要时设置宽图或单图 `width`。
...
- **访问方式**：题库 Query/Command Service 使用异步 SQLModel `AsyncSession`，由 `SessionDep` 每请求注入并关闭；写用例显式提交，异常由会话边界回滚。
...
- 真题写接口位于 `modules/exam/router.py`：创建/更新/删除为 POST，并由 `get_current_admin`保护；详情和查重当前分别使用公开查询路由。
- 模拟题写接口位于 `modules/mock/router.py`：创建/更新/删除为 POST 并由管理员保护；编辑详情、来源标题等读取由公开查询路由提供。
...
**说明**：4.3、4.4 的路径表是目标契约示例，不是当前路由清单；当前已实现路径以 4.1、`backend-fastapi/app/modules/exam/router.py`、`backend-fastapi/app/modules/mock/router.py` 和 `frontend/src/api/*.ts` 为准。
...
- `ExamManage.vue`/`MockManage.vue` 的分类筛选使用 `Select + filterable`，只允许选择当前科目已有分类；搜索输入不会作为新的分类值提交，编辑表单中的自由输入字段仍由 `InputSelect` 负责。

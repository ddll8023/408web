...
**事实冲突**：当前前端表单使用 camelCase 和 `categoryIds` 等字段，后端请求 Schema 主要定义 snake_case 的 `category` 字符串；`ExamEditDialog`、`MockEditDialog`和部分管理页面的请求形态不完全一致。当前后端也只在 Service 中做有限查重，没有对题型、选项 JSON、难度和分类归属进行完整业务校验。本文档将其作为现状风险，并给出统一目标契约。
...
- `ExamResponse.category`在后端转换为数组，`MockResponse.category`仍可能是 JSON 字符串；前端依赖 `request.js`和页面逻辑分别处理。
- 前端选择题校验主要在弹窗，后端当前没有同等强度的题型/选项结构校验。
- 当前数据库无题目复合唯一约束，应用层查重存在并发窗口。

**规范冲突**：现有创建/更新/删除已经是 POST，但查询详情/查重的部分路由和前端调用仍采用 GET；项目 `python-spec` 要求所有 Web API 使用 POST + Request Schema，且 Service 不返回响应包装。本设计目标契约按 POST-only 设计，现有事实单独列出，不视为已完成迁移。
...
- **初始化**：应用启动执行 `SQLModel.metadata.create_all`；当前没有数据库迁移链。结构变化按当前 Model 直接重建开发数据库，但删除/重建现有数据库必须先得到明确授权。
...
- 当前统一响应为 `Response[T]`，`code=200`表示成功；Service 返回 `ExamResponse`/`MockResponse`，路由包装响应。
- 当前前端编辑器调用 `frontend/src/api/exam.js`、`mock.js`；请求字段命名和分类/选项形态不完全统一，属于目标契约需修正的事实差异。

### 4.2 目标方法、响应和 DTO 规则

本设计的题目维护接口全部使用 POST，路径用动作语义区分，不通过 HTTP 方法区分操作。JSON 请求全部使用 Pydantic Request Schema；目标统一响应为 `ApiResponse[T]`，当前 `Response[T]`仅为现状记录。API 层使用 `response_model`及统一成功/错误构造，Service 不返回 `ApiResponse`或 ORM 实体。

...
- `ExamList.vue`按当前年份在浏览器拼接 Markdown 和 `docx`；`ExamClassify.vue`的科目导出通过直接下载链接访问后端 `/api/exam/export`，当前服务实际只生成 Markdown。
...
**规范冲突**：当前统计读取接口使用 GET + Query，前端直接下载使用 URL 查询参数；项目规范要求所有接口 POST + Request Schema，列表/聚合响应采用统一 `ApiResponse`。第 4 节只把目标契约设计为 POST，并保留当前已实现边界的事实说明。
...
- 应用启动调用 `SQLModel.metadata.create_all`，没有迁移链；本模块不改变 Schema，不执行数据库删除、重建或清理。
- 当前连接设置 `check_same_thread=False`，没有显式 WAL/外键开关；统一数据库层可按 `sqlite-ops` 规范补齐，但不在本模块单独配置。
...
- 实际前缀为 `/api`，当前统计查询主要为 GET + Query 参数。
- 真题已实现 `/api/exam/year-stats`、`/api/exam/category-stats` 和 `/api/exam/export`；模拟题已实现来源/科目/分类统计接口；分类模块已实现跨科目分类统计。
- 当前 JSON 成功响应是 `Response[T]`、`code=200`；真题分类统计响应没有统一去重总数字段，前端自行求和。
- 前端 `ExamCategoryStats.vue` 使用的 `/api/exam/category-stats/export` 当前后端未发现；这是待确认/待实现接口，不属于已实现能力。

### 4.2 目标契约与统一方法

所有新增或改动统计/导出接口均使用 POST。筛选条件必须放进 Pydantic Request Schema JSON Body，不使用 Query 参数；目标 JSON 响应使用 `ApiResponse[T]`，若未来返回分页明细使用 `ApiResponse[PaginatedResponse[T]]`，字段为 `lists` 和 `pagination`。文件导出成功时返回文件流，失败必须在开始流式响应前返回统一错误。
...
- `frontend/src/api/exam.js`、`mock.js`和`category.js`是 JSON 请求的统一入口；当前 `request.js`只集中转换响应，目标迁移时还需集中处理请求字段和文件 Blob，页面不得自行拼接下载 URL绕过鉴权和错误处理。
- 统计查询可取消或只接受最新响应；导出请求在浏览器侧应区分加载、下载、失败和取消。当前没有后台任务、事件或 SSE。

### 4.6 兼容与风险说明

当前统计页面的去重和导出能力存在公开行为差异：真题分类统计前端直接求和会重复计数，统计导出路径后端未实现，真题科目导出参数中的格式并未真正支持多格式。目标契约修正这些问题会影响前端数据读取和用户看到的总数，实施时必须先同步公共响应、统计口径和导出失败提示；本文档未修改业务代码。

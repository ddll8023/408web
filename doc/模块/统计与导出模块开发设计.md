...
**已确认事实**：不同分类统计入口仍有各自的数据来源，但真题分类统计接口已返回按题目 ID 去重的 `total_count`，并将分类引用总数单独返回；分类项按题目 ID 在单个分类内去重，一题多分类时可同时计入多个分类。
...
5. 统计管理页面按科目筛选后只显示该科目的统计；导出文件只包含当前筛选范围的汇总数据，不包含题目明细。
...
- `ExamCategoryStats.vue`读取真题分类统计，分别展示去重题目总数和分类引用总数，并通过 `api/exam.ts` 的 `requestBlob` 调用 `/api/exam/export-category-stats` 导出 Markdown/Excel。
...
- 当前 JSON 成功响应是 `ApiResponse[T]`、`code=200`；真题分类统计响应包含 `total_count`（题目 ID 去重）和 `category_reference_count`（分类引用总数），分类项包含题型拆分。
...
| 导出分类统计 | `POST /api/exam/export-category-stats` | `ExamCategoryStatsExportRequest { subject_id: int | null, format: Literal['markdown','xlsx'] }`。 | 只含分类汇总的 Markdown/Excel 文件流。 | ADMIN；请求内生成临时文件，不保存导出记录。 |
...
真题分类统计的去重总数和分类引用总数已在接口中分开表达，管理员可按科目导出只含汇总数据的 Markdown/Excel。真题科目导出当前只支持 Markdown，Word 由 `ExamList.vue` 在浏览器端生成。导出是请求内生成的临时文件，不记录下载历史；若题库规模导致响应时间或内存不可接受，仍需另行设计异步导出任务。

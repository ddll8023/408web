...
**已确认事实**：年份统计由 `ExamService.get_year_stats()`提供；真题分类统计由 `ExamService.get_category_stats()`和 `ExamCategoryService.get_category_stats()`分别提供；模拟题统计由 `MockService`提供；年份页面在浏览器生成 Markdown/Word，分类统计页面当前只展示统计结果，导出按钮明确标记为暂不可用，当前后端也未发现对应的分类统计导出路由。

**事实冲突**：不同统计入口的去重口径不一致。`MockService.get_category_stats()`显式按题目 ID 去重总量，`ExamCategoryService.get_category_stats()`返回按科目去重的分类题目总量，而 `ExamCategoryStats.vue` 当前对各分类 `count` 求和并按“分类引用总数”展示，遇到一题多分类时该数值会重复计入，不能当作去重题目总数。本文档把统一去重口径作为目标设计，不把当前页面结果误写成已统一。
...
| 方向 | 当前项目位置 | 协作内容 |
|---|---|---|
| 真题统计入口 | `backend-fastapi/app/api/exam.py`、`app/services/exam_query_service.py`、`app/services/exam_export_service.py` | 年份统计、分类统计、索引和按科目导出。 |
...
| 前端管理调用方 | `frontend/src/views/admin/ExamCategoryStats.vue`、`CategoryManage.vue` | 显示分类统计、科目筛选，并明确标识尚未实现的导出能力。 |
...
- `ExamCategoryStats.vue`读取真题分类统计，并把每个分类计数相加作为“分类引用总数”；页面的分类统计导出按钮当前为禁用状态，不再调用 `/api/exam/category-stats/export`，而 `exam.py`也未定义该路由。
- `CategoryManage.vue`的真题统计使用分类统计接口，模拟题统计使用模拟题科目/分类接口；动态模拟题分类不是统计表中的持久化分类。

**当前契约**：统计读取接口使用 POST + Request Schema，响应使用统一 `ApiResponse`；真题科目导出通过 `api/exam.ts` 的 `requestBlob` 请求文件流。分类统计导出仍属于待实现功能，当前页面只保留不可用状态提示，不宣称已经支持下载。
...
- 当前 JSON 成功响应是 `ApiResponse[T]`、`code=200`；真题分类统计响应仍没有统一去重总数字段，`ExamCategoryStats.vue` 将分类项求和后展示为分类引用总数，因此一题多分类时不会得到去重总题数。
- 分类统计导出接口当前后端未发现；`ExamCategoryStats.vue` 已将导出按钮置为不可用，不再调用历史的 `/api/exam/category-stats/export` 地址。目标接口仍属于待确认/待实现能力。
...
- `frontend/src/api/exam.ts`、`mock.ts`和`category.ts`是 JSON 请求的统一入口；`request.ts`集中校验响应信封、转换字段并区分 JSON/Blob。页面不得自行拼接已实现导出接口的下载 URL；分类统计导出仍是已知缺失接口，待后端契约落地后再接入统一 Blob 请求。
- 统计查询可取消或只接受最新响应；导出请求在浏览器侧应区分加载、下载、失败和取消。当前没有后台任务、事件或 SSE。

### 4.6 兼容与风险说明

当前统计页面仍存在两项实现缺口：真题分类统计接口没有提供去重总数字段，页面目前只能展示分类引用总数；分类统计导出接口尚未实现，但前端已将按钮置为不可用，不再把它作为可下载能力。真题科目导出当前只支持 Markdown，Word 由 `ExamList.vue` 在浏览器端生成。后续补齐缺口会影响前端数据读取和用户看到的总数，实施时必须先同步公共响应、统计口径和导出失败提示。

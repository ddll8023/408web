...
**已确认事实**：年份统计由 `modules/exam/query_service.py` 提供，真题分类统计和真题导出由 `modules/reporting/service.py` 提供，目录总量统计由 `CategoryQueryService` 提供；模拟题统计由 `modules/mock/query_service.py` 提供；年份页面在浏览器生成 Markdown/Word，管理员分类统计页面通过 reporting 导出 Service 生成只包含汇总数据的 Markdown/Excel。
...
- **不做项**：随机出题统计、学习进度、用户行为分析、定时统计快照、异步任务队列和 Word 服务端生成；分类统计 Excel 仅生成汇总工作表，不扩展为题目明细或复杂报表。历史 `exam_random_stat` 表当前没有对应 ORM 模型，也没有被本模块使用。
...
| 方向 | 当前项目位置 | 协作内容 |
|---|---|---|
| 真题统计入口 | `backend-fastapi/app/modules/exam/router.py`、`backend-fastapi/app/modules/exam/query_service.py`、`backend-fastapi/app/modules/reporting/router.py`、`backend-fastapi/app/modules/reporting/service.py` | 年份统计、分类统计、索引、按科目导出和管理员分类统计汇总导出。 |
| 分类统计入口 | `backend-fastapi/app/modules/catalog/routers/category.py`、`backend-fastapi/app/modules/catalog/category_query_service.py` | 跨科目分类总量、启用分类数和题目引用去重统计。 |
| 模拟题统计入口 | `backend-fastapi/app/modules/mock/router.py`、`app/modules/mock/query_service.py` | 来源、科目和动态分类统计。 |
...
| 导出/内容协作 | `frontend/src/components/business/ExamQuestionCard.vue`、`QuestionCopyMenu.vue`、`QuestionImageRenderer.vue`、`ExamList.vue` | 单题 Markdown/纯文本/PNG 图片复制和浏览器端 Markdown/Word 文档生成。 |
...
历史 `exam_random_stat` 表当前没有统计/导出调用方；它不属于本模块的数据契约，也不能被误用为学习统计。

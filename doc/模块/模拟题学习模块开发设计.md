...
| 后端入口 | `backend-fastapi/app/api/mock.py` | 暴露模拟题查询、统计、详情和管理员 CRUD。 |
| 业务/数据 | `backend-fastapi/app/services/mock_service.py`、`app/services/mock_query_service.py`、`app/services/mock_command_service.py`、`app/repositories/mock_repository.py`、`app/schemas/mock.py`、`app/models/entities.py` | 完成筛选、分页、动态分类聚合、查重和 `mock_question` 持久化。 |
| 依赖模块 | 账户与权限、学科目录、内容格式、图片资源、统计导出 | 提供管理员身份、科目/分类上下文、阅读渲染、图片访问和派生统计。 |

### 2.2 当前代码边界与冲突

后端 `mock.py` 直接创建 `MockService`，`MockService` 保留兼容门面，查询/写入 Service 通过 `MockRepository` 使用异步 SQLModel Session，查询 Service 在 `to_response()` 中补充科目和作者名称。`MockQueryParams` 定义分页、来源、分类、科目、无分类、关键词和排序字段，当前路由通过 POST JSON Body 接收这些字段。
...
- **访问方式**：`MockService` 使用异步 SQLModel `AsyncSession`；请求依赖负责 Session 生命周期，`MockCommandService` 显式提交，异常时由会话边界回滚。

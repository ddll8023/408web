...
后端 `mock.py` 直接创建 `MockService`，Service 使用异步 SQLModel Session 并在 `_to_response()` 中查询科目和作者名称；没有 Repository 层。`MockQueryParams` 定义分页、来源、分类、科目、无分类、关键词和排序字段，当前路由通过 POST JSON Body 接收这些字段。
...
- **并发写入**：创建/更新先由 Service 做友好查重，数据库模型和迁移脚本已提供 `title IS NOT NULL AND question_number IS NOT NULL` 条件下的 `(source, title, question_number)` 唯一索引；唯一冲突仍需按 409 处理。
...
- **初始化与结构变化**：应用启动执行 `SQLModel.metadata.create_all`；现有数据曾由 `scripts/migrate_high_priority.py` 和 `schema_migrations` 记录完成高优先级迁移。模型变化和已有数据库重建必须按授权规则处理。
...
| `question_type` | `TEXT` | 非空；默认 `ESSAY` | `CHOICE`/`ESSAY`，模型和迁移包含检查约束。 |
...
- 已确认模型声明 `(source, title, question_number)` 非空部分唯一索引；迁移脚本另建立 `subject_id`、`author_id` 普通索引。列表仍常按来源/科目筛选、更新时间或题号排序。
...
- 当前只有高优先级数据迁移脚本和 `schema_migrations` 记录，没有完整通用迁移框架；结构变化不能在未授权情况下删除或重建数据库。本文档不执行数据库删除、重建或清理。

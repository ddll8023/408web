...
5. 选择题选项解析成功时可作答并获得正确/错误反馈；主观题和无答案题不会出现误导性的选择题反馈，题目内容支持 Markdown、纯文本和“题目+选项、答案、全部”三种 PNG 图片复制。
...
5. `ExamQuestionCard.vue`本地维护当前选项和答案显隐；这些状态不会写入 `mock_question`。复制图片时由 `QuestionImageRenderer.vue` 按“题目+选项、答案、全部”三种范围生成 PNG，不截取交互控件。

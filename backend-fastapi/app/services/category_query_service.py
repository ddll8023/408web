...
class CategoryQueryService:
    ...
    async def get_category_stats(
        ...
        for subject in await self.repository.list_subjects():
            ...
            question_count = await self.repository.count_questions_with_categories(
                subject.id,
                question_type,
            )
    ...
    async def check_category_usage(self, category_id: int) -> int:
        ...
        return (
            await self.repository.count_question_references(
                category.subject_id,
                category.name,
                "exam",
            )
    ...
    async def get_category_usage(self, category_id: int) -> ExamCategoryUsageResponse:
        category = await self._get_category_or_raise(category_id)
        question_count = await self.repository.count_question_references(
            category.subject_id,
            category.name,
            "exam",
        )
    ...
    async def _apply_question_counts(
        ...
        for subject_categories in categories_by_subject.values():
            rows = await self.repository.list_question_categories(
                subject_categories[0].subject_id,
                question_type,
            )
            question_ids_by_category: dict[str, set[int]] = defaultdict(set)
            for row in rows:
                for name in set(parse_categories(row[1])):
                    question_ids_by_category[name].add(row[0])

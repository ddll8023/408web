...
from web408.core.exceptions import ConflictException, NotFoundException
from web408.modules.catalog.schemas.subject import (
    SubjectCreateRequest,
    SubjectUpdateRequest,
    SubjectResponse
)
from web408.modules.catalog.subject_repository import SubjectRepository
...
class SubjectService:
    ...
    async def get_enabled_subjects(self) -> List[SubjectResponse]:
        ...
        rows = await self.repository.list_enabled()
        question_counts = await self.exam_read_service.count_by_subject(
            {subject.id for subject in rows}
        )

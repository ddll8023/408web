"""模拟题模块的底层公开读取边界，不依赖目录或 HTTP 响应编排。"""
...
class MockCategoryReferenceRead:
    """目录统计所需的模拟题分类引用，只携带标识和原始 JSON。"""
...
class MockReadService:
    ...
    async def list_question_categories(self, subject_id: int) -> list[MockCategoryReferenceRead]:
        """转换为不可变引用 DTO，不向外暴露 ORM 或数据库行。"""
        rows = await self.repository.list_question_categories(subject_id)
        return [MockCategoryReferenceRead(id=row.id, category=row.category) for row in rows]
